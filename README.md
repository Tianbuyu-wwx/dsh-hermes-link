...
- **Hermes→DSH producer plugin** (v0.6.3): the consumer above had nothing to consume until the Hermes side existed. `npx hermes-link-install-hermes-plugin` copies `hermes-plugin/dsh-outbox` into `<Hermes Home>/plugins/dsh-outbox/`, where Hermes discovers it like any out-of-tree plugin. It then drops one `import` notification per Hermes **turn end** (so DSH imports/refreshes the session immediately instead of waiting for its own dump watcher), a `notify` for failed/interrupted turns, and a `producer_ready` ping at load — plus `/dsh-notify <message>` inside Hermes for a human-driven message. **Restart Hermes after installing** (plugins load at start-up); `GET /mcp/collab/doctor` reports whether the producer is present.
