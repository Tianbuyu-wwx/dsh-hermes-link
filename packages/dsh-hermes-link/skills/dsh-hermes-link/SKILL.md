...
- `npx dsh-hermes-link-setup [--dry-run] [--json] [--hermes-home <dir>] [--url <dsh url>]` (v0.6.10) — the one-command onboarding: inspect → install/refresh the Hermes plugin → enable it in Hermes → report the restarts still owed. Pure planning lives in `services/setup.mjs` (tests cover the whole decision table).
