...
when_to_use: |
  ...
  tools (callable from this session):
    ...
    - rotate_outbox_now            — v0.3.3 F2: force an immediate outbox file rotation pass (size + age archive + purge); Hermes cron can call this hourly
    - dispatch_status             — v0.3.3 F4: list live continuable dispatch children with status / tokens / recent audit entries; filterable by task_id
...
7. **Shared conversation record** — Hermes writes `~/.dsh/hermes-inbox/session.jsonl`
   ...
   **The main DSH session does NOT auto-inject these turns on session-start
   (since v0.3.3)** — they are available on demand only.
...
10. **Outbox file rotation (v0.3.3 F2)** — `usage.jsonl` / `session-mirror/<sid>.jsonl`
...
11. **Live dispatch status (v0.3.3 F4)** — `dispatch_status` (JSON-RPC + DSH-side tool)
...
12. **Write-behind outbox queue (v0.3.3 E2)** — `appendUsage` / `appendSessionEvent` /
...
- `GET  /mcp/collab/stream` (v0.3.3 F1) — `text/event-stream` of real-time events
...
- `GET /mcp/collab/metrics` (v0.3.3 F6) — Prometheus text exposition format (text/plain; version=0.0.4). Returns 16 counters + 8 gauges. Bearer auth same as main routes. Suitable for Prometheus / Grafana Agent scraping at 15s intervals.
- `dispatch_dry_run` JSON-RPC tool (v0.3.3 F5) — pre-flight estimator. Returns estimated prompt/output tokens + would_block_on + warnings. Heuristic (chars/4). Use before dispatch_task to validate token budgets and surface unknown skills without spawning a sub-agent.
