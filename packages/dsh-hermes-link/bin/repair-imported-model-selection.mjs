...
//
// USAGE
//   npx hermes-link-repair-model-pins                       # dry run
//   npx hermes-link-repair-model-pins --apply
//   npx hermes-link-repair-model-pins --provider X --model Y [--effort Z]
//   npx hermes-link-repair-model-pins --id hermes-20260710_232907_343d1e --apply
//
// SHIPPED WITH THE PACKAGE (v0.6.1): exposed as the `hermes-link-repair-model-pins`
// bin; the repo's scripts/repair-imported-model-selection.mjs is a thin wrapper.
//
...
/**
 * Fold the SAME rule DSH applies (dsh-api-session-controller's
 * applyModelSelectionProjection): lastUsed comes from the last request/header,
 * and a model/selection event overrides it until a request/header matches it.
 * The effective route is `pending ?? lastUsed`.
 *
 * Why this replaced a stricter "the LAST event must be a model/selection" check:
 * the moment a repaired session is opened in DSH, resuming appends its own
 * events (session/end-seed, turn boundaries), so the pin stops being the tail
 * while remaining perfectly effective -- the tail test reported a healthy
 * session as broken. What actually matters is the route the projection resolves
 * to, which is what this returns.
 * @returns {{provider: string|null, model: string|null}}
 */
export function effectiveModelRoute(events) {
  let lastUsed = null
  let pending = null
  for (const e of events || []) {
    if (!e || typeof e !== 'object') continue
    if (e.type === 'model/selection' && e.data && typeof e.data === 'object') { pending = e.data; continue }
  ...
  const effective = pending || lastUsed || { provider: null, model: null }
  return { provider: effective.provider || null, model: effective.model || null }
}

/** The synthetic provider the converter used to record: served by no adapter. */
export const SYNTHETIC_PROVIDER = 'dsh-hermes-link'
...
async function main() {
  ...
  for (const t of targets) {
    ...
    try {
      ...
      const effective = effectiveModelRoute(events)
      if (!FORCE && effective.provider && effective.provider !== SYNTHETIC_PROVIDER) {
        report.alreadyPinned++
        console.log('  OK      ' + t.id + '  (already routes to ' + effective.provider + '/' + effective.model + ')')
        continue
      }
      const event = {
