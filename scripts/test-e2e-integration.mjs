...
//
// v0.4.0 - End-to-end integration test that wires up the HTTP surface
...
await t('e2e: /mcp/collab (GET) responds to ping with version', async () => {
  ...
  try {
    ...
    assert.equal(body.result.version, '0.4.0')
...
await t('e2e: route table covers all documented paths', async () => {
  ...
  try {
    const expected = [
      ...
      '/mcp/collab/health',
      '/mcp/collab/sessions',
'/mcp/collab/session-stream',
    '/mcp/collab/session-mirror/status',
      '/mcp/collab/import',
